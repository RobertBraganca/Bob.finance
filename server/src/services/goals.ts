 ...
 * `Promise.all`, mesmo padrão de `goalHistory`) e soma a META de cada mês
 * que TEM meta configurada (um mês sem meta não conta como meta zero, nem
 * pra baixo nem pra cima — mesma regra de "sem meta configurada" que uma
 * meta mensal ausente já lê como `no_target`). Sem meta em nenhum mês do
 * ano, o alvo anual também é `null`, não zero.
 */
export async function getYearProgress(year: string, accountId?: number | null): Promise<YearProgress> {
  ...
  const perMonth = await Promise.all(periods.map((period) => getPeriodProgress(period, accountId)))
